...
export class HomeUserPage implements OnInit {
    @ViewChild("map_home", { static: false }) mapElement: ElementRef;
    map: any;
    ...
    async GetUsers(latitude, longitude) {
        ...
        this.allServicesService
            .getData(
                "get_user_role_wise/?type=home&role=barber&latitude=" +
                latitude +
                "&longitude=" +
                longitude
            )
    ...
    async initMap() {
        ...
        google.maps.event.addListener(this.map, "idle", () => {
            ...
            this.show_marker_on_map(this.latitude, this.longitude);
        });
    }

    show_marker_on_map(latitude, longitude) {
        if (latitude != "" && longitude != "") {
            ...
            let title = this.user.first_name + ' ' + this.user.last_name;
            let image = this.user.userImage;
    ...
    gotoprofile() {
        let url = 'userprofile/' + this.user.user_id + '/private';
